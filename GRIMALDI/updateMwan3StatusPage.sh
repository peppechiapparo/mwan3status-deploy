...
# Ensure static assets (logo) are available under web root
...
if [ -n "$LOGO_SRC" ]; then
  cp -f "$LOGO_SRC" /www/mwan3/static/Telespazio.png
fi
...
if [ -f "$TEMPLATE_HTML" ]; then
  # single source-of-truth: copy the template into web root
  cp -f "$TEMPLATE_HTML" /www/mwan3/index.html
else
  # minimal placeholder to avoid serving broken page; user must restore template in repo
  cat > /www/mwan3/index.html <<'EOHTML'
<!doctype html>
<meta charset="utf-8">
<title>WAN Status - template missing</title>
<body>
Template missing. Put util/www/index.template.html in the project (or /opt/util/grimaldi/www/) and rerun this script.
</body>
EOHTML
fi

# VERSION handling: use WEBAPP_VERSION if set in the script, or allow an override from environment.
# The deploy process generates a machine-readable `/www/mwan3/version.json` file and injects a
...
cat > /www/mwan3/version.json <<EOF || true
{
  "version": "${VER}",
  "generated": "${GEN_TS}",
  "source": "deploy"
...
sed -i "s#__HOSTNAME__#$(printf '%s' "$CLEAN_NAME" | sed 's/[&/]/\\&/g')#g" /www/mwan3/index.html
